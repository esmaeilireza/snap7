...
class StatusIndicator(tk.Canvas):
    ...
    def __init__(self, parent, size=12, color=T.SUCCESS, **kwargs):
        super().__init__(parent, width=size, height=size, bg=T.BG_PANEL,
        ...
        self._dot_id = self.create_oval(1, 1, self.size - 1, self.size - 1,
                                        fill=self.color, outline=self.color)
        self._highlight_id = self.create_oval(
            self.size // 4, self.size // 4,
            self.size // 4 + self.size // 3, self.size // 4 + self.size // 3,
...
class Badge(tk.Canvas):
    def __init__(self, parent, text="0", color=T.DANGER, size=20, **kwargs):
        super().__init__(parent, width=size, height=size, bg=T.BG_DARK,
                         highlightthickness=0, **kwargs)
        self.text = text
        self.color = color
        self.size = size
    ...
    def _draw(self):
        ...
        self.create_text(self.size // 2, self.size // 2, text=str(self.text),
                         fill='white', font=('Tahoma', 9, 'bold'))


class MetricDisplay(tk.Frame):
    def __init__(self, parent, title="Metric", value="--", unit="",
                 value_color=T.PRIMARY, **kwargs):
        super().__init__(parent, bg=T.BG_PANEL, **kwargs)
        tk.Label(self, text=title.upper(), bg=T.BG_PANEL, fg=T.TEXT_SECONDARY,
                 font=T.FONT_XS).pack(anchor='w')
        value_frame = tk.Frame(self, bg=T.BG_PANEL)
        value_frame.pack(anchor='w', pady=(2, 0))
        self.value_label = tk.Label(value_frame, text=str(value), bg=T.BG_PANEL,
                                    fg=value_color, font=T.FONT_XXL)
        self.value_label.pack(side='left')
        if unit:
            tk.Label(value_frame, text=unit, bg=T.BG_PANEL, fg=T.TEXT_SECONDARY,
                     font=T.FONT_LARGE).pack(side='left', padx=(4, 0), pady=(14, 0))

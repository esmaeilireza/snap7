...
class LiveChartWidget(tk.Frame):
    def __init__(self, parent, title="Temperature (°C)", max_points=60,
                 setpoint=65.5, **kwargs):
    ...
    def _build_chart(self):
        ...
        # Setpoint line
        self.ax.axhline(y=self.setpoint, color=T.DANGER, linestyle='--',
                        linewidth=1.5, alpha=0.8)
        self.ax.text(0.98, self.setpoint, f"{self.setpoint:.2f} SETPOINT",
                     color=T.DANGER, fontsize=9, fontweight='bold',
                     transform=self.ax.get_yaxis_transform(),
                     ha='right', va='bottom',
                     bbox=dict(boxstyle='round,pad=0.3',
                               facecolor=T.DANGER_BG, edgecolor=T.DANGER, alpha=0.8))

        self.line, = self.ax.plot([], [], color=T.PRIMARY_GLOW, linewidth=2,
                                  label='Temperature')
        self.fill = None
        ...
        self.canvas.get_tk_widget().pack(fill='both', expand=True,
                                         padx=T.PADDING_MD, pady=T.PADDING_SM)
        self.canvas.draw()
    ...
    def _build_controls(self):
        ...
        for tf in timeframes:
            btn = tk.Button(controls, text=tf,
    ...
    def _auto_scale(self):
        ...
        if self.temperatures:
            self.ax.set_ylim(min(self.temperatures) - 2,
                             max(self.temperatures) + 2)
    ...
    def add_point(self, temperature, timestamp=None):
        ...
        if self.fill is not None:
            self.fill.remove()
        self.fill = self.ax.fill_between(ts_list, temp_list,

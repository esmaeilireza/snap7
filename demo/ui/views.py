...
FALLBACK = {
    'BG_DARK': '#0F172A', 'BG_NAVY': '#1E293B', 'BG_PANEL': '#1E293B',
    'BG_HOVER': '#334155', 'PRIMARY': '#38BDF8', 'PRIMARY_DARK': '#0C4A6E',
    'PRIMARY_GLOW': '#7DD3FC', 'SUCCESS': '#22C55E', 'SUCCESS_DARK': '#14532D',
    'WARNING': '#F59E0B', 'DANGER': '#EF4444', 'DANGER_BG': '#450A0A',
    'TEXT_PRIMARY': '#E2E8F0', 'TEXT_SECONDARY': '#94A3B8',
    'TEXT_MUTED': '#64748B', 'TEXT_DIM': '#475569',
    'BORDER': '#334155', 'BORDER_ACTIVE': '#38BDF8',
...
class ViewManager(tk.Frame):
    ...
    def __init__(self, parent, **kwargs):
        ...
        self.grid_rowconfigure(0, weight=1)
        self.grid_columnconfigure(0, weight=1)
        self._views = {}       # name -> widget instance (cached)
        self._factories = {}   # name -> callable(parent) -> BaseView
...
class AssetsView(BaseView):
    ...
    def __init__(self, parent, asset_panel=None, **kwargs):
        ...
        header.grid(row=0, column=0, sticky='ew', padx=_c('PADDING_MD'),
                    pady=_c('PADDING_SM'))
        tk.Label(header, text="ASSET TABLE", bg=_c('BG_PANEL'), fg=_c('PRIMARY'),
                 font=_c('FONT_TITLE')).pack(side='left', padx=_c('PADDING_SM'))
        ...
        btn_bar.grid(row=2, column=0, sticky='ew', padx=_c('PADDING_MD'),
                     pady=_c('PADDING_SM'))
        tk.Button(btn_bar, text="REFRESH FROM PANEL", bg=_c('PRIMARY_DARK'),
                  fg=_c('PRIMARY'), font=_c('FONT_SMALL'), relief='flat',
                  takefocus=True, command=self.refresh_from_panel).pack(side='left')
    ...
    def refresh(self, rows):
        """rows: iterable of (tag, type, zone, status, last_value, updated)."""
    ...
    def _render(self):
        ...
        rows = sorted(self._rows, key=lambda r: str(r[self.COLUMNS.index(self._sort_col)]),
...
class DataMonitorView(BaseView):
    ...
    def __init__(self, parent, **kwargs):
        ...
        self.grid_rowconfigure(1, weight=1)
        self.grid_columnconfigure(0, weight=1)
        tk.Label(self, text="LIVE DATA MONITOR", bg=_c('BG_DARK'),
                 fg=_c('PRIMARY'), font=_c('FONT_TITLE')).grid(
            row=0, column=0, sticky='w', padx=_c('PADDING_MD'), pady=_c('PADDING_SM'))
    ...
    def update_tags(self, data):
        """data: {tag: {'value':.., 'unit':.., 'quality':..}} or {tag: value}."""
        ...
        for tag, info in data.items():
            ...
            if tag in self._items and self.tree.exists(self._items[tag]):
                ...
                self._items[tag] = self.tree.insert('', 'end', values=row,
                                                    tags=(quality,))
...
class AlarmsView(BaseView):
    ...
    def __init__(self, parent, **kwargs):
        ...
        self.grid_rowconfigure(1, weight=1)
        self.grid_columnconfigure(0, weight=1)
        tk.Label(self, text="ALARM MANAGEMENT", bg=_c('BG_DARK'),
                 fg=_c('DANGER'), font=_c('FONT_TITLE')).grid(
            row=0, column=0, sticky='w', padx=_c('PADDING_MD'),
            pady=_c('PADDING_SM'))
        frame = tk.Frame(self, bg=_c('BG_DARK'))
        frame.grid(row=1, column=0, sticky='nsew', padx=_c('PADDING_MD'))
        frame.grid_rowconfigure(0, weight=1)
        ...
        btn_bar.grid(row=2, column=0, sticky='ew', padx=_c('PADDING_MD'),
                     pady=_c('PADDING_SM'))
        tk.Button(btn_bar, text="ACKNOWLEDGE SELECTED", bg=_c('PRIMARY_DARK'),
                  fg=_c('PRIMARY'), font=_c('FONT_SMALL'), relief='flat',
                  takefocus=True, command=self.acknowledge_selected).pack(side='left')
        tk.Button(btn_bar, text="ACK ALL", bg=_c('BG_NAVY'), fg=_c('TEXT_SECONDARY'),
                  font=_c('FONT_SMALL'), relief='flat', takefocus=True,
                  command=self._ack_all).pack(side='left', padx=_c('PADDING_SM'))

    def add_alarm(self, severity='WARNING', source='PLC', message='Alarm',
...
class TrendsView(BaseView):
    ...
    def __init__(self, parent, chart_factory=None, **kwargs):
        super().__init__(parent, **kwargs)
        self._chart_factory = chart_factory  # callable(parent) -> chart widget
        self.grid_rowconfigure(1, weight=1)
        self.grid_columnconfigure(0, weight=1)

        top = tk.Frame(self, bg=_c('BG_DARK'))
        top.grid(row=0, column=0, sticky='ew', padx=_c('PADDING_MD'),
                 pady=_c('PADDING_SM'))
        tk.Label(top, text="TAG:", bg=_c('BG_DARK'), fg=_c('TEXT_SECONDARY'),
                 font=_c('FONT_NORMAL')).pack(side='left')
        self.tag_var = tk.StringVar()
        self.tag_combo = ttk.Combobox(top, textvariable=self.tag_var,
                                      values=['TEMP_01'], width=20, state='readonly')
        self.tag_combo.pack(side='left', padx=_c('PADDING_SM'))
        for tf in self.TIMEFRAMES:
            tk.Button(top, text=tf, bg=_c('BG_NAVY'), fg=_c('PRIMARY'),
                      font=_c('FONT_SMALL'), relief='flat', takefocus=True,
                      command=lambda t=tf: self._select_tf(t)).pack(side='left', padx=2)
        self._active_tf = '5m'
        ...
        if chart_factory is not None:
            ...
            self.chart = None
            tk.Label(chart_holder, text="No chart available", bg=_c('BG_PANEL'),
                     fg=_c('TEXT_MUTED')).grid(row=0, column=0)
    ...
    def push_point(self, tag, value):
        """Add a data point; reuses the existing ChartWidget.add_point."""
...
class ReportsView(BaseView):
    def __init__(self, parent, **kwargs):
        ...
        form.grid(row=0, column=0, sticky='ew', padx=_c('PADDING_MD'),
                  pady=_c('PADDING_SM'))
        tk.Label(form, text="GENERATE REPORT", bg=_c('BG_PANEL'),
                 fg=_c('PRIMARY'), font=_c('FONT_TITLE')).grid(
            row=0, column=0, columnspan=4, sticky='w', pady=(0, _c('PADDING_SM')))
        tk.Label(form, text="From (YYYY-MM-DD):", bg=_c('BG_PANEL'),
                 fg=_c('TEXT_SECONDARY'), font=_c('FONT_SMALL')).grid(row=1, column=0, sticky='w')
        ...
        tk.Label(form, text="To (YYYY-MM-DD):", bg=_c('BG_PANEL'),
                 fg=_c('TEXT_SECONDARY'), font=_c('FONT_SMALL')).grid(row=1, column=2, sticky='w')
        ...
        tk.Label(form, text="Report type:", bg=_c('BG_PANEL'),
                 fg=_c('TEXT_SECONDARY'), font=_c('FONT_SMALL')).grid(row=2, column=0, sticky='w',
                                                                     pady=(_c('PADDING_SM'), 0))
        self.type_var = tk.StringVar(value='Temperature History')
        ttk.Combobox(form, textvariable=self.type_var, takefocus=True,
                     values=['Temperature History', 'Alarm Summary',
                             'Data Quality', 'Full Export'],
                     state='readonly', width=24).grid(row=2, column=1, sticky='w',
                                                      padx=_c('PADDING_SM'),
                                                      pady=(_c('PADDING_SM'), 0))
        tk.Button(form, text="GENERATE CSV", bg=_c('PRIMARY_DARK'), fg=_c('PRIMARY'),
                  font=_c('FONT_SMALL'), relief='flat', takefocus=True,
                  command=self.generate_csv).grid(row=2, column=2, sticky='w',
                                                  pady=(_c('PADDING_SM'), 0))
        form.grid_columnconfigure(1, weight=1)
        form.grid_columnconfigure(3, weight=1)

        self.status_var = tk.StringVar(value='')
        tk.Label(self, textvariable=self.status_var, bg=_c('BG_DARK'),
                 fg=_c('SUCCESS'), font=_c('FONT_SMALL')).grid(
            row=1, column=0, sticky='w', padx=_c('PADDING_MD'))

        frame = tk.Frame(self, bg=_c('BG_DARK'))
        frame.grid(row=2, column=0, sticky='nsew', padx=_c('PADDING_MD'),
                   pady=_c('PADDING_SM'))
        frame.grid_rowconfigure(0, weight=1)
        frame.grid_columnconfigure(0, weight=1)
...
class SettingsView(BaseView):
    DEFAULTS = {'host': '127.0.0.1', 'port': 102, 'unit_id': 1,
                'poll_interval_ms': 500, 'simulation': True, 'theme': 'Industrial Dark'}
    ...
    def __init__(self, parent, on_save=None, **kwargs):
        ...
        form = tk.Frame(self, bg=_c('BG_PANEL'))
        form.grid(row=0, column=0, sticky='nsew', padx=_c('PADDING_MD'),
                  pady=_c('PADDING_MD'))
        form.grid_columnconfigure(1, weight=1)

        def add_row(r, label):
            tk.Label(form, text=label, bg=_c('BG_PANEL'), fg=_c('TEXT_SECONDARY'),
                     font=_c('FONT_SMALL')).grid(row=r, column=0, sticky='w',
                                                 pady=4, padx=_c('PADDING_SM'))
            return r + 1
        ...
        ttk.Combobox(form, textvariable=self.theme_var, takefocus=True,
                     values=['Industrial Dark', 'Light'], state='readonly').grid(
            row=r - 1, column=1, sticky='ew', padx=_c('PADDING_SM'), pady=4)

        btns = tk.Frame(form, bg=_c('BG_PANEL'))
        btns.grid(row=r, column=0, columnspan=2, sticky='ew',
                  padx=_c('PADDING_SM'), pady=_c('PADDING_MD'))
        tk.Button(btns, text="SAVE", bg=_c('PRIMARY_DARK'), fg=_c('PRIMARY'),
                  font=_c('FONT_SMALL'), relief='flat', takefocus=True,
                  command=self._save).pack(side='left')
        tk.Button(btns, text="RESET DEFAULTS", bg=_c('BG_NAVY'),
                  fg=_c('TEXT_SECONDARY'), font=_c('FONT_SMALL'), relief='flat',
                  takefocus=True, command=self._reset).pack(side='left',
                                                            padx=_c('PADDING_SM'))
    ...
    def _save(self):
        ...
        if interval < 50:
            messagebox.showerror("Invalid Interval",
                                 "Poll interval must be at least 50 ms.")
            return
        settings = {

...
class AssetPanel(tk.Frame):
    ...
    def _build_header(self):
        tk.Label(
            self,
            text="\U0001f3ed CONNECTED ASSETS",
            bg=T.BG_PANEL,
            fg=T.PRIMARY,
            font=T.FONT_TITLE,
    ...
    def _open_add_asset_dialog(self):
        ...
        tk.Label(dialog, text="ADD NEW ASSET", bg=T.BG_PANEL, fg=T.PRIMARY, font=T.FONT_TITLE).pack(
            pady=(T.PADDING_MD, T.PADDING_SM)
        )
        ...
        for i, (label, default) in enumerate(field_defs):
            tk.Label(form, text=label, bg=T.BG_PANEL, fg=T.TEXT_SECONDARY, font=T.FONT_SMALL).grid(
            ...
            tk.Entry(
                ...
                font=T.FONT_SMALL,
                relief="flat",
            ).grid(row=i, column=1, sticky="ew", padx=T.PADDING_SM, pady=4)
            fields[label] = var
        ...
        def on_submit():
            ...
            if name in self.assets:
                tk.messagebox.showwarning(
                    "Duplicate", f"Asset '{name}' already exists.", parent=dialog
                )
                return
            ...
            self.add_asset(
                name,
                ip,
                status if status in AssetItem.STATUS_COLORS else "ONLINE",
